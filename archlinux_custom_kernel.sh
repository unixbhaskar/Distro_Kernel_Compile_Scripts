...
srcver=$(grep "_srcver=" PKGBUILD)

sed -i "s/$(echo $srcver)/_srcver=$(echo $kernel)-arch1 /" PKGBUILD

#cn=$(echo $kernel | cut -d"." -f1-2)

sed -i "s/_srcname=archlinux-linux/_srcname=$(echo $kernel)-arch1 /" PKGBUILD
...
sed -i '20d' PKGBUILD
sed -i '20i \"$_srcname::https://git.archlinux.org/linux.git/snapshot/\$_srcver.tar.xz\"' PKGBUILD
...
sudo pacman -U  --noconfirm $(hostname)-$kernel-$kernel.arch1-1-x86_64.pkg.tar.xz  

sudo pacman -U --noconfirm $(hostname)-$kernel-headers-$kernel.arch1-1-x86_64.pkg.tar.xz

sudo pacman -U  --noconfirm $(hostname)-$kernel-docs-$kernel.arch1-1-x86_64.pkg.tar.xz 
...
>$EFIBOOTENTRY/ArchLinux.conf
echo "title ArchLinux" | sudo tee -a $EFIBOOTENTRY/ArchLinux.conf 
...
"\EFI\ArchLinux\vmlinuz-$(hostname)-$kernel  root=PARTUUID=e79c9191-3302-4dc8-a62a-5cc22d266643  loglevel=3  systemd.show_status=true rw initrd=\EFI\ArchLinux\initramfs-$(hostname)-$kernel.img" | sudo tee -a /boot/efi/EFI/archlinux.nsh
